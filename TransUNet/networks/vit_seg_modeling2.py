...
class Attention(nn.Module):
    ...
    def forward(self, hidden_states):
        ...
        temps[0].append(query_layer.cpu().detach().numpy())
        temps[1].append(key_layer.cpu().detach().numpy())
        temps[2].append(value_layer.cpu().detach().numpy())
        ...
        temp.append(attention_probs.cpu().detach().numpy())

        # temp=self.softmax(attention_scores / math.sqrt(self.attention_head_size))
        # global temps
        # tempNumpy=temp.cpu().detach().numpy()
        # temps.append(tempNumpy)
...
class DecoderCup(nn.Module):
    ...
    def forward(self, hidden_states, features=None):
        ...
        # x:4,768,196 hidden_feature  196_patchesNum_14*14 786_hidden

        x = x.contiguous().view(B, hidden, h, w)
        x = self.conv_more(x)   # d_h_w -> 512_h_w
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        # 此处features为Encoder层层传过来的，用作decoder的残差连接
